...
public class Loja {
	...
	public void atualizarEstoque(Compra compra) {
		for(int indice =0; indice < compra.getListaProdutos().size(); indice++) {
			Produto produto = compra.getListaProdutos().get(indice);
			produtosLoja[produto.getCodigo() - 1].tirarEstoque(compra.getListaQuantidade().get(indice));
			produto.getCodigo();
		}
	}
	*/
